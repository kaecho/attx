...
fn render_plugins_js(plugins: &[Value]) -> Result<String> {
    // Compact JSON like RPG Maker exports (single-line objects).
    let body = serde_json::to_string(plugins)?;
    // Pretty-ish: one plugin per line for smaller diffs
    let mut lines = Vec::new();
    lines.push("// Generated by RPG Maker.".to_string());
    lines.push("// Do not edit this file directly.".to_string());
    lines.push("// Modified by attx (plugin parameter translations only).".to_string());
    lines.push("var $plugins =".to_string());
...
pub fn extract_plugins(content_root: &Path, source_lang: &str) -> Result<Vec<TextUnit>> {
    ...
    for (idx, plug) in plugins.iter().enumerate() {
        let status = plug.get("status").and_then(|v| v.as_bool()).unwrap_or(false);
        if !status {
            continue;
        }
        let name = plug
...
fn is_machine_literal(s: &str) -> bool {
    ...
    if !needs_translation(t, "ja") && (t.contains('/') || t.ends_with(".png") || t.ends_with(".ogg"))
    {
        return true;
    }
    false
    ...
    match v {
        ...
        Value::Object(map) => {
            for (k, el) in map {
                ...
                match el {
                    Value::String(s) => {
                        if force || needs_translation(s, source_lang) {
                            extract_value_strings(
                                s,
                                base_loc,
                                plugin,
