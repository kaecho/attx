...
`detect` → `init` → `extract` → `status` → 試し `translate --limit 20` → `learn note`（試し訳で再利用できる癖が見えたときだけ）→ 本番 `translate` → `review` → `writeback`（インプレースフォーマットでは最初にドライラン）。各段階の後、件数と次のステップを報告します。
