...
`detect` → `init` → `extract` → `status` → trial `translate --limit 20` → `learn note` (only if the trial showed a reusable habit) → full `translate` → `review` → `writeback` (dry-run first for in-place formats). After each stage, report counts and the next step.
